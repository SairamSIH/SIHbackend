...
def create_supabase_folder(unique_id):
    ...
    vid_res = clients.storage.from_('sihbucketbackend').upload(f'{unique_id}/Videos/supabasetry.txt', file=os.path.join('DataMan', 'supabasetry.txt'))
    docs_res = clients.storage.from_('sihbucketbackend').upload(f'{unique_id}/Documents/supabasetry.txt', file=os.path.join('DataMan', 'supabasetry.txt'))
